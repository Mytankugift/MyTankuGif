  ...
  async createOrder(input: CreateOrderInput): Promise<OrderResponse> {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        isGiftOrder: input.isGiftOrder || false,
        giftSenderId: input.giftSenderId || null,
        giftRecipientId: input.giftRecipientId || null,
    ...
    const addressUserId = input.isGiftOrder && input.giftRecipientId 
      ? input.giftRecipientId 
      : input.userId;

    // Buscar si ya existe una dirección con los mismos datos para este usuario (o destinatario si es regalo)
    ...
    if (!address) {
      address = await prisma.address.create({
        data: {
          userId: addressUserId, // Usar destinatario si es regalo, sino el usuario
    ...
    // ✅ Eliminar productos de wishlists del usuario (excepto la automática "Me gusta")
    try {
      const productIds = input.items.map(item => item.productId);
      const variantIds = input.items.map(item => item.variantId).filter(Boolean) as string[];
      
      // Obtener todas las wishlists del usuario excepto la automática
      const userWishlists = await prisma.wishList.findMany({
        where: {
          userId: input.userId,
          isAutoGenerated: false, // ✅ Excluir la wishlist "Me gusta"
        },
        include: {
          items: {
            where: {
      ...
      // Eliminar items de las wishlists
      for (const wishlist of userWishlists) {
        if (wishlist.items.length > 0) {
          ...
          console.log(`✅ [ORDERS] Eliminados ${itemIds.length} producto(s) de wishlist "${wishlist.name}"`);
        }
      }
    } catch (error) {
      // No fallar la orden si hay error eliminando de wishlists
      console.error('⚠️ [ORDERS] Error eliminando productos de wishlists:', error);
    }
