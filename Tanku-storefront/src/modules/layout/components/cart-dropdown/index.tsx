...
const CartDropdown = ({
  ...
  // El backend ya calcula item.total y subtotal, usar esos valores directamente
  // Calcular desde items usando item.total (que viene del backend)
  const calculatedSubtotal = cartState?.items?.reduce((sum, item) => {
    // El backend ya calcula item.total = unit_price * quantity
    // Usar item.total si está disponible, sino calcular desde unit_price
  ...
  const baseSubtotal = (cartState?.subtotal && cartState.subtotal > 0) ? cartState.subtotal : calculatedSubtotal
