...
const WishListDropdown = ({ productId, productTitle }: WishListDropdownProps) => {
  ...
  const checkProductInWishLists = (currentWishLists: WishList[]) => {
    if (!productId) return {}
    try {
      if (!Array.isArray(currentWishLists)) {
        console.warn("checkProductInWishLists: currentWishLists is not an array", currentWishLists)
        return {}
      }
      ...
      currentWishLists.forEach((list: WishList) => {
        ...
        const hasProduct =
          Array.isArray(list.products) && list.products.some((product) => product?.id === productId) || false
  ...
  const handlerRetriverWishList = async () => {
    ...
    try {
      ...
      if (!Array.isArray(wishListsData)) {
        console.warn("handlerRetriverWishList: wishListsData is not an array", wishListsData)
        return
      }
