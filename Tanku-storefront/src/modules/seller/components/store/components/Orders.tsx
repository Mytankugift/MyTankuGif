...
import { getSellerOrders, SellerOrder } from '../actions/get-seller-orders'

interface OrdersProps {
  customer: string
}

const Orders: React.FC<OrdersProps> = ({ customer }) => {
  ...
  const loadOrders = async () => {
    try {
      setLoading(true)
      const sellerOrders = await getSellerOrders(customer)
  ...
  const getTotalSellerAmount = (order: SellerOrder) => {
    return order.orderVariants.reduce((total, variant) => {
      return total + Number(variant.original_total)
    }, 0)
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-2xl font-bold text-[#73FFA2]">ÓRDENES DE MI TIENDA</h2>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="bg-gray-700 p-8 rounded-lg">
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <p className="text-gray-400 text-sm">Cuando los clientes compren tus productos, aparecerán aquí.</p>
          </div>
        </div>
      ) : (
              ...
              <tbody className="bg-gray-700 divide-y divide-gray-600">
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-600 transition-colors">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-[#73FFA2]">
                      {formatCurrency(getTotalSellerAmount(order))}
                    </td>
              ...
              <div key={order.id} className="bg-gray-600 rounded-lg p-4">
                ...
                <div className="flex justify-between items-center mb-3">
                  <div className="text-sm text-gray-300">
                    {order.orderVariants.length} producto(s) míos
                  </div>
                  <div className="text-sm font-medium text-[#73FFA2]">
                    {formatCurrency(getTotalSellerAmount(order))}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-400">Total Orden:</span>
                      <span className="text-white font-medium">{formatCurrency(selectedOrder.total_amount)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Mis Ventas:</span>
                      <span className="text-[#73FFA2] font-medium">{formatCurrency(getTotalSellerAmount(selectedOrder))}</span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div>
                  <h4 className="text-sm font-medium text-[#73FFA2] mb-2">Información del Cliente</h4>
                  <div className="text-sm text-gray-300">
                  ...
                  <h4 className="text-sm font-medium text-[#73FFA2] mb-2 mt-4">Dirección de Envío</h4>
                  <div className="text-sm text-gray-300">
              ...
              <div>
                <h4 className="text-sm font-medium text-[#73FFA2] mb-4">Mis Productos en esta Orden</h4>
                <div className="space-y-3">
                  {selectedOrder.orderVariants.map((variant) => (
                    <div key={variant.id} className="flex justify-between items-center p-3 bg-gray-700 rounded-lg">
                      <div>
                        <div className="text-sm font-medium text-white">
