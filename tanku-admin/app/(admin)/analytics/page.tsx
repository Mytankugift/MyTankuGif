...
type TabId = 'overview' | 'sales' | 'users' | 'gifts' | 'support' | 'operations' | 'catalog' | 'social'
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <KpiCard label="Ganancia real" value={formatCurrency(s.profit)} showTrend={false} icon={ArrowTrendingUpIcon} hint="dropi_dropshipper_win" />
        ...
        <KpiCard label="Costo de envío Dropi" value={formatCurrency(s.shippingCost)} showTrend={false} icon={TruckIcon} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ChartCard title="Top productos vendidos" subtitle="Por unidades">
          <RankingTable items={data.topProducts} valueLabel="Unidades" extraLabel="Ingresos" extraFormatter={formatCurrency} />
        </ChartCard>
        <ChartCard title="Top categorías" subtitle="Por unidades">
          <RankingTable items={data.topCategories} valueLabel="Unidades" extraLabel="Ingresos" extraFormatter={formatCurrency} />
        </ChartCard>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard label="Regalos creados" value={formatNumber(s.created)} showTrend={false} icon={GiftIcon} />
        <KpiCard label="Tasa de aceptación" value={formatPercent(s.acceptanceRate)} showTrend={false} hint={`${formatNumber(s.accepted)} aceptados`} />
        <KpiCard label="Tasa de pago" value={formatPercent(s.paidRate)} showTrend={false} />
        <KpiCard
          label="Tiempo medio a aceptar"
          value={s.avgTimeToAcceptHours !== null ? `${s.avgTimeToAcceptHours.toString().replace('.', ',')} h` : '—'}
          showTrend={false}
        />
      </div>

      <ChartCard title="Regalos creados" subtitle="En el periodo">
        <TimeSeriesChart
          data={data.series}
          height={320}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ChartCard title="Regalos por estado">
          <DistributionDonut data={data.byEstado} labels={GIFT_ESTADO_LABELS} />
        </ChartCard>
        <ChartCard title="Top regaladores" subtitle="Por número de regalos">
          <RankingTable items={data.topSenders} valueLabel="Regalos" />
        </ChartCard>
      </div>
    </div>
  )
