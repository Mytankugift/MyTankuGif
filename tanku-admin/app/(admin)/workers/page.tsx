...
export default function WorkersDashboard() {
  ...
  // Redirigir a login si no está autenticado (solo después de hidratación)
  useEffect(() => {
    if (!hasHydrated) return
    
    if (!isAuthenticated) {
      console.log('[WORKERS] No autenticado, redirigiendo a login')
      router.push('/auth/login')
    }
  }, [hasHydrated, isAuthenticated, router])

  // Cargar jobs al montar y cada 5 segundos (solo si está autenticado y hidratado)
  useEffect(() => {
  ...
  }, [hasHydrated, isAuthenticated, loadJobs, loadProxyStatus])

  // Polling para jobs activos (solo si está autenticado y hidratado)
  useEffect(() => {
  ...
  // Retornar null si no está hidratado o no está autenticado (después de todos los hooks)
